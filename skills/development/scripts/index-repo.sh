...
SUMMARY="$INDEX_DIR/SUMMARY.md"
{
  ...
  echo "## Index Files"
  echo ""
  echo "| File | Description | How to Search |"
  echo "|------|-------------|---------------|"
  echo "| \`file-tree.txt\` | All files with sizes | Use the Grep tool for 'pattern' in \`.claude/repo-index/file-tree.txt\` |"
  echo "| \`dir-tree.txt\` | Directory structure | Use the Read tool directly |"
  echo "| \`symbols.txt\` | Functions, classes, types + file:line | Use the Grep tool for 'SymbolName' in \`.claude/repo-index/symbols.txt\` |"
  echo "| \`dependencies.txt\` | Import map between files | Use the Grep tool for 'module' in \`.claude/repo-index/dependencies.txt\` |"
  echo ""
