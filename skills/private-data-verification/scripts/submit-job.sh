#!/bin/bash
# Job submission script for Private Data Verification
# Usage: ./submit-job.sh "your private data here"

SELLER_WALLET="0x228F7097fB812828a2F08EE29bAC0c58f9e0Bb63"
OFFERING="private-data-verification"
DATA="$1"

if [ -z "$DATA" ]; then
  echo "Usage: ./submit-job.sh \"your private data here\""
  exit 1
fi

echo "Submitting private data verification job..."
JOB_RESPONSE=$(acp job create "$SELLER_WALLET" "$OFFERING" \
  --requirements "{\"cpdv_data\": \"$DATA\"}" \
  --json)
